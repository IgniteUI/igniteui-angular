...
When you focus a specific cell and press one of the following key combinations, the described bahaviour should be expected:
    ...
    - `Tab` - sequentially move the focus over the next cell and if the last cell in the row is reached move to next row. If next row is group row the whole row is focused, if it is data row, move focus over the first element, which can be checkbox if row is selectable or a cell;
    - `Shift + Tab` - sequentially move focus to the previous cell in the row, if the first cell is reached and the row is selectable move the focus to the checkbox, if not move the focus to the previous row and focus the group row or a last cell of the previous row;
...
For more detailed information see the [official keyboard navigation spacification](https://github.com/IgniteUI/igniteui-angular/wiki/igxGrid-Specification#kb-navigation).
