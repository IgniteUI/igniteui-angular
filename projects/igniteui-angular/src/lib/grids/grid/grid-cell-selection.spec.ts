...
describe('IgxGrid - Cell selection #grid', () => {
    ...
    describe('Base', () => {
        ...
        it('Should be able to select a range with mouse dragging', () => {
            ...
            for (let i = 3; i < 5; i++) {
                const cell = grid.gridAPI.get_cell_by_index(i, grid.columnsCollection[i - 1].field);
            ...
            for (let i = 2; i >= 0; i--) {
                const cell = grid.gridAPI.get_cell_by_index(0, grid.columnsCollection[i].field);
                UIInteractions.simulatePointerOverElementEvent('pointerenter', cell.nativeElement);
                detect();
    ...
    describe('Keyboard navigation', () => {
        ...
        it('Should be able to select and move scroll with arrow keys and holding Shift', (async () => {
            ...
            GridSelectionFunctions.verifyCellSelected(cell);
            for (let i = 3; i < 6; i++) {
                cell = grid.gridAPI.get_cell_by_index(1, grid.columnsCollection[i - 1].field);
                UIInteractions.triggerEventHandlerKeyDown('arrowright', gridContent, false, true);
                await wait(100);
                fix.detectChanges();
            }
            ...
            for (let i = 5; i > 0; i--) {
                cell = grid.gridAPI.get_cell_by_index(0, grid.columnsCollection[i - 1].field);
