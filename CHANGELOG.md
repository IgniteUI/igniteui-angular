...
- `IgxButton`
    - **Behavioral Change** `buttonSelected` event is now emitted not only when a button gets selected, but also when it gets unselected. A benefit of that is when updating the value bound to the `selected` input of an `IgxButton`, the button group in which the button resides is now able to update the styling of the button from selected to deselected. If this event was used in a scenario where it is assumed that the button gets selected, is a good idea the logic to be branched now based on `eventArgs.selected` condition.
