...
describe('IgxGrid - Row Selection #grid', () => {
    ...
    describe('Base tests', () => {
        ...
        it('Header checkbox should select/deselect all rows', () => {
            ...
            spyOn(grid.rowSelectionChange, 'emit').and.callThrough();
            ...
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledTimes(1);
            ...
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledWith(args);
            ...
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledTimes(2);
            args = {
        ...
        it('Header checkbox should deselect all rows - scenario when clicking first row, while header checkbox is clicked', () => {
            ...
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledTimes(4);
        });
        ...
        it('Checkbox should select/deselect row', () => {
            ...
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledTimes(3);
            args = {
                added: [],
                cancel: false,
                event: jasmine.anything() as any,
        ...
        it('Should select the row with mouse click ', () => {
            ...
            expect(grid.selectedRows).toEqual([2]);
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledTimes(1);
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledWith({
                added: [2],
                cancel: false,
                event: mockEvent,
                newSelection: [2],
        ...
        it('Should NOT select multiple rows with Shift + Click when selectRowOnClick has false value', () => {
            ...
            expect(grid.selectedRows).toEqual([]);
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledTimes(0);
            GridSelectionFunctions.verifyRowSelected(secondRow, false);
        ...
        it('Should be able to cancel rowSelectionChange event', () => {
            const firstRow = grid.getRowByIndex(0);
            grid.rowSelectionChange.subscribe((e: IRowSelectionEventArgs) => {
                e.cancel = true;
            });
        ...
        it('Should be able to programmatically overwrite the selection using rowSelectionChange event', () => {
            const firstRow = grid.getRowByIndex(0);
            const secondRow = grid.getRowByIndex(1);
    ...
    describe('Integration tests', () => {
        ...
        it('Filtering: Should select correct rows when filter is applied', () => {
            ...
            GridSelectionFunctions.verifyHeaderRowCheckboxState(fix);
            expect(grid.rowSelectionChange.emit).toHaveBeenCalledTimes(6);
