...
import { ICellEditDoneEventArgs, ICellEditEventArgs, IRowEditDoneEventArgs, IRowEditEventArgs } from '../common/events';
...
describe('IgxGrid - Row Editing #grid', () => {
    ...
    describe('General tests', () => {
        ...
        it('Emit all events with proper arguments', () => {
            ...
            expect(row.inEditMode).toBe(false);
            let cellEditExitArgs: ICellEditDoneEventArgs = {
                cellID: cell.id,
                rowID: cell.row.key,
                rowKey: cell.row.key,
                rowData: cell.row.data,
            ...
            const rowEditExitArgs: IRowEditDoneEventArgs = {
                rowID: row.key,
                key: row.key,
                rowData: initialRowData,
                data: initialRowData,
            ...
            cellEditExitArgs = {
                ...
                data: Object.assign({}, row.data, { ProductName: newCellValue }),
                oldValue: cell.value,
            ...
            const cellDoneArgs: ICellEditDoneEventArgs = {
                rowID: cell.row.key,
                rowKey: row.key,
                cellID: cell.id,
            ...
            const rowDoneArgs: IRowEditDoneEventArgs = {
                rowID: row.key,
                key: row.key,
                rowData: updatedRowData, // with rowEditable - IgxGridRowEditingComponent
                data: updatedRowData,
    ...
    describe('Integration', () => {
        ...
        it(`Summaries: Should update summaries after row editing completes`, fakeAsync(() => {
            ...
            cellElem = grid.gridAPI.get_cell_by_index(0, 'OrderDate');
            UIInteractions.simulateDoubleClickAndSelectEvent(cellElem);
            tick(16);
            // Cell will always be first
    ...
    describe('Events', () => {
        ...
        it(`Should properly emit 'rowEditExit' event - Filtering`, () => {
            ...
            const cellInput = (cellElem as any).nativeElement.querySelector('[igxinput]');
    ...
    describe('Transaction', () => {
        ...
        it('cellEditDone, rowEditDone should emit the committed/new rowData', () => {
            ...
            expect(grid.cellEditDone.emit).toHaveBeenCalledWith(cellDoneArgs);
            expect(grid.rowEditDone.emit).toHaveBeenCalledWith(rowDoneArgs);
            const rowDoneSpyArgs = rowDoneSpy.calls.mostRecent().args[0] as IRowEditDoneEventArgs;
        ...
        it('Transaction Update, Delete, Add, Undo, Redo, Commit check transaction and grid state', () => {
            ...
            state = trans.getAggregatedChanges(false);
            expect(state.length).toEqual(0);
            expect((cellElem as any).nativeElement.classList).not.toContain(ROW_EDITED_CLASS);
        });
        ...
        it('Should allow to change value of a cell with initial value of false', () => {
            ...
            cellElem = grid.gridAPI.get_cell_by_index(3, 'InStock');
        ...
        it('Should allow to change of a cell in added row in grid with transactions', () => {
            ...
            cellElem = grid.gridAPI.get_cell_by_index(10, 'ProductName');
