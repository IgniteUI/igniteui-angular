...
- `igxOverlay`:
    - **Behavioural Change** - `igxOverlay` - no longer persists element scrolling `out of the box`. In order to persist an element scroll position after attaching the element to an overlay, handle the exposed onPositioned overlay event and manage/restore the scroll position.
