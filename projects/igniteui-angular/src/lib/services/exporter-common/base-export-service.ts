...
export abstract class IgxBaseExporter {
    ...
    private mapHierarchicalGridColumns(island: any, gridData: any) {
        ...
        if (island.autoGenerate) {
            ...
            /*
            * If there is a registered child grid then get its columns because the collection of columns within
            * the rowIsland might be outdated. If no then get the row islands column because they will be up to date.
            */
            const rowIslandGrid = island.rowIslandAPI.getChildGrids()[0];
            const islandColumnList = rowIslandGrid?.columns || island.columns;
