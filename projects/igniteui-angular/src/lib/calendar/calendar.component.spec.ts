        ...
        describe('Rendered Component - ', () => {
            ...
            it('Week numbers should appear as first column', () => {
                ...
                calendarRows.forEach((row, idx) => {
                    ...
                    if (idx === 0) {
                        ...
                        expect(firstRowItem.firstChild.innerText).toEqual((idx).toString());
                    }
                });
            });
            ...
            it('should be displayed the correct week numbers in the first column', () => {
                ...
                calendarRowsMar.forEach((row, idx) => {
                    ...
                    if (idx === 6) {
                        expect(firstRowItem.firstChild.innerText).toEqual('14');
                    }
                });
                ...
                calendarRowsDec.forEach((row, idx) => {
                    const firstRowItem = row.nativeElement.children[0];
                    if (idx === 6) {
                        expect(firstRowItem.firstChild.innerText).toEqual('1');
                    }
                });
            });
            ...
            fit('Calendar date should persist the focus when select date in the (next/prev) month.', fakeAsync(() => {
                const component = dom.query(By.css(HelperTestFunctions.CALENDAR_CSSCLASS));
                const calendarMonth = calendar.daysView.getCalendarMonth;
                let value = calendarMonth[0][4];

                UIInteractions.triggerKeyDownEvtUponElem('Tab', component.nativeElement);
                ...
                // flush();
                //
                // console.log(document.activeElement, date, value);
                // debugger;
