...
export class IgxGridAPIService {
    ...
    protected state: Map<string, IgxVirtGridComponent> = new Map<string, IgxVirtGridComponent>();

    public register(grid: IgxVirtGridComponent) {
        this.state.set(grid.id, grid);
    }

    public get(id: string): IgxVirtGridComponent {
    ...
    public markForCheck(id: string): void {
        ...
        if (this.get(id).rowList) {
            this.get(id).rowList.forEach((row) => row.changeDet.markForCheck());
        }
    }
