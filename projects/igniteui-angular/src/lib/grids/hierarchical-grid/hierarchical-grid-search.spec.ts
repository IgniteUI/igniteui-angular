...
describe('IgxHierarchicalGrid ', () => {
    ...
    beforeEach(async(() => {
        TestBed.configureTestingModule({
            declarations: [
                ...
                IgxHierarchicalGridSearchMultiRITestBaseComponent
            ],
            imports: [
    ...
    describe('Search with sibling row islands - ', () => {
        beforeEach(async(() => {
            fixture = TestBed.createComponent(IgxHierarchicalGridSearchMultiRITestBaseComponent);
            fixture.detectChanges();
            hierarchicalGrid = fixture.componentInstance.hgrid;
        }));

        it('should order matchInfoCache correctly when there are results in sibling child grids.', async () => {
            (hierarchicalGrid.getRowByIndex(1) as IgxHierarchicalRowComponent).toggle();
            (hierarchicalGrid.getRowByIndex(0) as IgxHierarchicalRowComponent).toggle();
            hierarchicalGrid.cdr.detectChanges();
            ...
            const res = hierarchicalGrid.lastSearchInfo.matchInfoCache;

            expect(res[0].row).toBe(hierarchicalGrid.data[0]);
            expect(res[1].row).toBe(hierarchicalGrid.data[0].childData[0]);
            expect(res[2].row).toBe(hierarchicalGrid.data[0].childData2[0]);
            expect(res[3].row).toBe(hierarchicalGrid.data[1].childData[0]);
            expect(res[4].row).toBe(hierarchicalGrid.data[1].childData2[0]);
        });
    });
});
...
export class IgxHierarchicalGridSearchMultiRITestBaseComponent extends IgxHierarchicalGridSearchTestBaseComponent {
