import { IGX_PREFIX, prefixResource } from 'igniteui-i18n-core';
import { CalendarResourceStringsES as ACalendarResourceStrings } from 'igniteui-i18n-resources';

/**
 * Spanish resource strings for IgxCalendar
 * 
 * Note:  This is an old implementation that uses outdated API for resource management.
 * We recommend using the new API and resources from 'igniteui-i18n-resources'.
 */
export const CalendarResourceStringsES = prefixResource(IGX_PREFIX, ACalendarResourceStrings);
