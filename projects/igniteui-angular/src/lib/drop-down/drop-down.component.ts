...
export class IgxDropDownComponent extends IgxDropDownBaseDirective implements IDropDownBase, OnChanges, AfterViewInit, OnDestroy {
    ...
    @ContentChild('scrollContainer.children', { static: true })
    protected dropdownContent: any;
    ...
    public get emptyClass(): boolean {
        if (this.collapsed) {
            return false;
        } else {
            return this.items.length === 0 && this.dropdownContent;
        }
    }
