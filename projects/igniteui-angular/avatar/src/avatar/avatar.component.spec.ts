import { Component, Signal, Type, viewChild } from '@angular/core';
import { ComponentFixture, TestBed } from '@angular/core/testing';
import { By } from '@angular/platform-browser';
import { IgxAvatarComponent, IgxAvatarType, IgxAvatarSize } from './avatar.component';
import { describe, it, expect, beforeEach } from 'vitest';

describe('Avatar', () => {
    const AvatarCSS = {
        round: `igx-avatar--rounded`,
        circle: `igx-avatar--circle`,
        small: `igx-avatar--small`,
        medium: `igx-avatar--medium`,
        large: `igx-avatar--large`,
        image: `igx-avatar--image`,
        initials: `igx-avatar--initials`,
        icon: `igx-avatar--icon`
    };

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [
                InitAvatarComponent,
                AvatarWithAttribsComponent,
                IgxAvatarComponent,
                InitIconAvatarComponent,
                InitImageAvatarComponent
            ]
        }).compileComponents();
    });

    it('Initializes avatar with auto-incremented id', () => {
        const { fixture, avatar, hostElement} = createComponent(InitAvatarComponent);

        expect(avatar.id).toContain('igx-avatar-');
        expect(hostElement.id).toContain('igx-avatar-');

        avatar.id = 'customAvatar';
        detectChanges(fixture);

        expect(avatar.id).toBe('customAvatar');
        expect(hostElement.id).toBe('customAvatar');
    });

    it('Initializes square and round avatar', () => {
        const { fixture, avatar, hostElement} = createComponent(AvatarWithAttribsComponent);

        expect(avatar.shape).toEqual('square');
        expect(hostElement.classList).not.toContain(AvatarCSS.circle);
        expect(hostElement.classList).not.toContain(AvatarCSS.round);

        avatar.shape = "circle";
        detectChanges(fixture);

        expect(avatar.shape).toEqual('circle');
        expect(hostElement.classList).toContain(AvatarCSS.circle);
        expect(hostElement.classList).not.toContain(AvatarCSS.round);

        avatar.shape = "rounded";
        detectChanges(fixture);

        expect(avatar.shape).toEqual('rounded');
        expect(hostElement.classList).toContain(AvatarCSS.round);
        expect(hostElement.classList).not.toContain(AvatarCSS.circle);
    });

    it('Can change its size', () => {
        const { fixture, avatar } = createComponent(AvatarWithAttribsComponent);

        expect(avatar.size).toEqual(IgxAvatarSize.SMALL);

        avatar.size = IgxAvatarSize.MEDIUM;
        detectChanges(fixture);

        expect(avatar.size).toEqual(IgxAvatarSize.MEDIUM);

        avatar.size = IgxAvatarSize.LARGE;
        detectChanges(fixture);

        expect(avatar.size).toEqual(IgxAvatarSize.LARGE);

        avatar.size = 'nonsense' as any;
        detectChanges(fixture);

        expect(avatar.size).toEqual(IgxAvatarSize.SMALL);
    });

    it('Initializes default avatar', () => {
        const { avatar, hostElement } = createComponent(InitAvatarComponent);

        expect(avatar.initials).toBeUndefined();
        expect(avatar.src).toBeUndefined();
        expect(avatar.icon).toBeUndefined();
        expect(avatar.shape).toEqual('square');

        expect(hostElement.textContent).toEqual('TEST');
    });


    it('Initializes initials avatar', () => {
        const { avatar, hostElement } = createComponent(AvatarWithAttribsComponent);

        expect(avatar.type).toEqual(IgxAvatarType.INITIALS);
        expect(avatar.initials).toEqual('ZK');
        expect(hostElement.querySelector('span').textContent).toEqual('ZK');
        expect(hostElement.classList).toContain(AvatarCSS.initials);
    });

    it('Initializes icon avatar', () => {
        const { avatar, hostElement } = createComponent(InitIconAvatarComponent);

        expect(avatar.type).toEqual(IgxAvatarType.ICON);
        expect(avatar.icon).toBeTruthy();
        expect(hostElement.classList).toContain(AvatarCSS.icon);
    });

    it('Initializes image avatar', () => {
        const { avatar, hostElement } = createComponent(InitImageAvatarComponent);

        expect(avatar.type).toEqual(IgxAvatarType.IMAGE);
        const image = hostElement.children[0];
        expect(image).toBeTruthy();
        expect(image.style.backgroundImage).toBeDefined();

        expect(image.classList).toContain(`igx-avatar__image`);
        expect(hostElement.classList).toContain(AvatarCSS.image);
    });

    it('Sets ARIA attributes', () => {
        const { avatar, hostElement } = createComponent(InitImageAvatarComponent);

        expect(avatar.roleDescription).toEqual('image avatar');
        expect(hostElement.getAttribute('role')).toEqual('img');
        expect(hostElement.getAttribute('aria-roledescription')).toEqual('image avatar');
        expect(hostElement.getAttribute('aria-label')).toEqual('avatar');
    });

    it('Normalizes the value of the `src` input', () => {
        const { fixture, avatar } = createComponent(InitImageAvatarComponent);

        avatar.src = "/assets/Test - 17.jpg";
        detectChanges(fixture);

        expect(avatar.src).toEqual("/assets/Test%20-%2017.jpg");
    });

    it('should not throw error if src is null', () => {
        const { fixture, avatar } = createComponent(InitImageAvatarComponent);

        expect(() => {
            avatar.src = null;
            detectChanges(fixture);
        }).not.toThrow();
    });

    it('avatar with [src] and fallback [initials] should not throw error if src is null', () => {
        const { fixture, avatar } = createComponent(AvatarWithAttribsComponent);

        expect(avatar.type).toEqual(IgxAvatarType.INITIALS);
        expect(avatar.initials).toEqual('ZK');
        expect(() => {
            avatar.src = null;
            detectChanges(fixture);
        }).not.toThrow();
    });
});

@Component({
    template: `<igx-avatar>TEST</igx-avatar>`,
    imports: [IgxAvatarComponent],
})
class InitAvatarComponent {
    public avatar = viewChild.required(IgxAvatarComponent);
}

@Component({
    template: `<igx-avatar [initials]="initials" size="small"></igx-avatar>`,
    imports: [IgxAvatarComponent]
})
class AvatarWithAttribsComponent {
    public avatar = viewChild.required(IgxAvatarComponent);
    public initials = 'ZK';
}

@Component({
    template: `<igx-avatar icon="person"></igx-avatar>`,
    imports: [IgxAvatarComponent]
})
class InitIconAvatarComponent {
    public avatar = viewChild.required(IgxAvatarComponent);
}

@Component({
    template: `<igx-avatar [src]="source"></igx-avatar>`,
    imports: [IgxAvatarComponent]
})
class InitImageAvatarComponent {
    public avatar = viewChild.required(IgxAvatarComponent);
    public source = 'data:image/jpeg;base64,';
}


function createComponent<T extends { avatar: Signal<IgxAvatarComponent> }>(component: Type<T>) {
    const fixture = TestBed.createComponent(component);
    fixture.detectChanges();

    return {
        fixture,
        avatar: fixture.componentInstance.avatar(),
        hostElement: fixture.debugElement.query(By.directive(IgxAvatarComponent)).nativeElement
    };
}

function detectChanges<T>(fixture: ComponentFixture<T>) {
    fixture.changeDetectorRef.detectChanges();
}
