...
describe("IgxGrid - GroupBy Unit tests", () => {

    beforeEach(async(() => {
        TestBed.configureTestingModule({
            declarations: [
                DefaultGroupByComponent
            ],
            imports: [
    ...
    it("should group a column when API is called in default state grid", () => {
        const fix = TestBed.createComponent(DefaultGroupByComponent);
        fix.detectChanges();
        fix.componentInstance.groupBy("City");
        expect(fix.componentInstance.checkColumnGrouped("City")).toBeTruthy();
    });
});

export class GroupByComponentBase {
    @ViewChild("grid1")
    public grid1: IgxGridComponent;

    groupBy(name: string, direction = SortingDirection.Asc, ignoreCase = true) {
		this.grid1.groupBy(name, direction, ignoreCase);
    }
    
    checkColumnGrouped(name: string, direction = SortingDirection.Asc, ignoreCase = true) {
        // TODO
        return true;
    }
}
...
export class DefaultGroupByComponent extends GroupByComponentBase {
