...
@Component({
    ...
    styleUrls: ['grid-height-test.sample.css'],
    templateUrl: 'grid-height-test.sample.html'
})

export class GridHeightTestComponent implements OnInit {
    ...
    public ngOnInit(): void {
        ...
        this.data = [
            ...
            { 'ID': 'BONAP', 'CompanyName': 'Bon app\s', 'ContactName': 'Laurence Lebihan', 'ContactTitle': 'Owner', 'Address': '12, rue des Bouchers', 'City': 'Marseille', 'Region': null, 'PostalCode': '13008', 'Country': 'France', 'Phone': '91.24.45.40', 'Fax': '91.24.45.41' },
    ...
    setComfortable(grid) {
        this.grids[grid].displayDensity = DisplayDensity.comfortable;
    }

    setCosy(grid) {
        this.grids[grid].displayDensity = DisplayDensity.cosy;
    }

    setCompact(grid) {
        this.grids[grid].displayDensity = DisplayDensity.compact;
    }

    toggleSummaries(grid) {
        const g = this.grids[grid],
            col = g.getColumnByName("CompanyName");
        col.hasSummary = !col.hasSummary;
        g.recalculateSummaries();
    }
}
