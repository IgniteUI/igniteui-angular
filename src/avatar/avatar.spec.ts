import{
    async,
    TestBed
} from '@angular/core/testing';
import { Component, ViewChild } from '@angular/core';
import { By } from '@angular/platform-browser';
import { AvatarModule, Avatar } from './avatar';


describe('Avatar', function(){
    beforeEach(async(() => {
        TestBed.configureTestingModule({
            declarations: [
                InitAvatar,
                AvatarWithAttribs,
                Avatar,
                InitIconAvatar,
                InitImageAvatar
            ]
        })
        .compileComponents();
    }));

    it('Initializes initials avatar', () => {
        let fixture = TestBed.createComponent(InitAvatar);
        fixture.detectChanges();
        expect(fixture.debugElement.query(By.css('img'))).toBeTruthy();
    });

    it('Initializes binded initials avatar', () => {
        let fixture = TestBed.createComponent(AvatarWithAttribs);
        fixture.detectChanges();
        expect(fixture.debugElement.query(By.css('img'))).toBeTruthy();
    });
    it('Initializes icon avatar', () => {
        let fixture = TestBed.createComponent(InitIconAvatar);
        fixture.detectChanges();
        let iconEl = fixture.debugElement.query(By.css('span>i')).nativeElement;
        expect(iconEl).toBeTruthy();
    });

    it('Initializes icon avatar', () => {
        let fixture = TestBed.createComponent(InitImageAvatar);
        fixture.detectChanges();
        let imageEl = fixture.debugElement.query(By.css('img')).nativeElement;
        expect(imageEl.getAttribute("src").length != 0).toBeTruthy();
    });
});

@Component({ template: `<ig-avatar initials="PP" size="medium" roundShape="false"
                            bgColor="paleturquoise">
                        </ig-avatar>`})
class InitAvatar {

}

@Component({ template: `<ig-avatar [initials]="initials" [bgColor]="bgColor"
    [roundShape]="roundShape"></ig-avatar>`})
class AvatarWithAttribs {
    initials: string = 'ZK';
    bgColor: string = 'lightblue';
    roundShape: string = "false";
}

@Component({ template: `<ig-avatar roundShape="true" icon="person"
                            bgColor="#0375be" size="someIncorectSize">
                        </ig-avatar>`})
class InitIconAvatar {

}


@Component({ template: `<ig-avatar roundShape="true" bgColor="#0375be"
                            [src]="source">
                        </ig-avatar>`})
class InitImageAvatar {
    source: string = 'data:image/jpeg;base64,'

}