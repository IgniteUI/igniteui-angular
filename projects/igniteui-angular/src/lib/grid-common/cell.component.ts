...
export class IgxGridCellComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    public update(val: any) {
        ...
        this.gridAPI.update_cell_implementation(this.gridID, rowSelector, this.cellID.columnID, val);
        this.cdr.markForCheck();
    ...
    public onKeydownArrowLeft(event) {
        ...
        if (columnIndex >= 0) {
             ...
             */
            if (bVirtSubscribe) {
                this.grid.focusNextCell(this.rowIndex, columnIndex, 'left', event);
            }
        }
    }
    ...
    public onKeydownCtrlArrowLeft(event) {
        ...
        this.grid.scrollTo(this.rowIndex, columnIndex);
    ...
    public onKeydownArrowRight(event) {
        ...
        if (columnIndex > -1 && columnIndex <= visibleColumns.length - 1) {
             ...
             */
            if (bVirtSubscribe) {
                this.grid.focusNextCell(this.rowIndex, columnIndex, 'right', event);
            }
        }
    }
