...
describe("IgxCalendar", () => {
    ...
    it("Calendar Model API", () => {
        ...
        expect(firstWeek[0].date.toDateString()).toMatch(new Date(2017, 4, 28).toDateString());
        expect(lastWeek[lastWeek.length - 1].date.toDateString()).toMatch(new Date(2017, 6, 1).toDateString());

        // 2017 June with first day set to Sunday
        let dates = calendar.monthdates(2017, 5);
        expect(dates[0].date.toDateString()).toMatch(new Date(2017, 4, 28).toDateString());
        expect(dates[dates.length - 1].date.toDateString()).toMatch(new Date(2017, 6, 1).toDateString());
        ...
        expect(dates[0].date.toDateString()).toMatch(new Date(2017, 4, 28).toDateString());
        expect(dates[dates.length - 1].date.toDateString()).toMatch(new Date(2017, 6, 8).toDateString());
        ...
        expect(dates[0].date.toDateString()).toMatch(new Date(2017, 4, 26).toDateString());
        expect(dates[dates.length - 1].date.toDateString()).toMatch(new Date(2017, 6, 6).toDateString());
        expect(dates.length).toEqual(42);

        // Leap year tests - 2016
        calendar.firstWeekDay = WEEKDAYS.SUNDAY;
        dates = calendar.monthdates(2016, 1);
        expect(dates[0].date.toDateString()).toMatch(new Date(2016, 0, 31).toDateString());
        expect(dates[dates.length - 1].date.toDateString()).toMatch(new Date(2016, 2, 5).toDateString());
        expect(dates.length).toEqual(35);
    });
    ...
    it("Calendar Model utility functions", () => {
        ...
        expect(calendar.timedelta(startDate, "day", 7).toDateString())
            .toEqual(calendar.timedelta(startDate, "week", 1).toDateString());
        newDate = calendar.timedelta(startDate, "day", -3);
        expect(newDate.getFullYear()).toEqual(2016);
    ...
    it("Calendar DOM structure", () => {
        ...
        expect(dom.queryAll(By.css(".igx-calendar__body-row > span")).length).toEqual(49);

        // Today class applied
        expect(
            dom.query(By.css(".igx-calendar__date--current"))
    ...
    it("Calendar selection - single with event", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
        ...
        // Select 14th
        weekDiv.triggerEventHandler("click", { target: weekDays[3].nativeElement });

        fixture.detectChanges();

        expect(calendar.onSelection.emit).toHaveBeenCalled();
        expect((calendar.value as Date).toDateString()).toMatch(nextDay.toDateString());
        expect(weekDays[3].nativeElement.classList.contains("igx-calendar__date--selected")).toBe(true);
        expect((fixture.componentInstance.model as Date).toDateString()).toMatch(nextDay.toDateString());
        expect(target.nativeElement.classList.contains("igx-calendar__date--selected")).toBe(false);
    }));

    it("Calendar selection - outside of current month - 1", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
        ...
        const parent = dom.query(By.css(".igx-calendar__body-row:last-child"));
        const target = parent.queryAll(By.css("span")).pop();

        parent.triggerEventHandler("click", { target: target.nativeElement });
        fixture.detectChanges();

        expect((fixture.componentInstance.model as Date).toDateString())
            .toMatch(new Date(2017, 6, 8).toDateString());
        expect(dom.query(By.css(".igx-calendar__header-date")).nativeElement.textContent.includes("Jul")).toBe(true);
    }));

    it("Calendar selection - outside of current month - 2", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
        ...
        expect(dom.query(By.css(".igx-calendar__header-date")).nativeElement.textContent.includes("May")).toBe(true);
    }));

    it("Calendar selection - single through API", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
    ...
    it("Calendar selection - multiple with event", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
        ...
        weekDays.forEach((el) => {
            weekDiv.triggerEventHandler("click", { target: el.nativeElement });
            fixture.detectChanges();
        });
        ...
        weekDays.forEach((el) => {
            expect(el.nativeElement.classList.contains("igx-calendar__date--selected")).toBe(true);
        });

        // Deselect last day
        weekDiv.triggerEventHandler("click", { target: weekDays[weekDays.length - 1].nativeElement });
        fixture.detectChanges();

        expect((calendar.value as Date[]).length).toEqual(6);
        expect((fixture.componentInstance.model as Date[]).length).toEqual(6);
        expect(weekDays[weekDays.length - 1].nativeElement.classList.contains("igx-calendar__date--selected"))
            .toBe(false);
    }));

    it("Calendar selection - multiple through API", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
        ...
        expect((fixture.componentInstance.model as Date[])[0].toDateString())
        ...
        // 11th June
        expect(weekDays[0].nativeElement.classList.contains("igx-calendar__date--selected"))
            .toBe(true);
        // 12th June
        expect(weekDays[1].nativeElement.classList.contains("igx-calendar__date--selected"))
            .toBe(true);

        // TODO: Multiple dates deselect on overlapping
    }));

    it("Calendar selection - range with event", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
        ...
        weekDiv.triggerEventHandler("click", { target: weekDays[0].nativeElement });
        fixture.detectChanges();
    ...
    it("Calendar selection - range through API", fakeAsync(() => {
        const fixture = TestBed.createComponent(IgxCalendarRenderingComponent);
        fixture.detectChanges();
        tick();
        fixture.detectChanges();
        ...
        for (const i of [0, 1, 2, 3]) {
            expect(weekDays[i].nativeElement.classList.contains("igx-calendar__date--selected")).toBe(true);
        }
    }));
});
