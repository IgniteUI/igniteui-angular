...
export class IgxPivotRowPipe implements PipeTransform {
    ...
    public transform(
        ...
        return rowStrategy.process(data, enabledRows, config.values, cloneStrategy, pivotKeys, this.grid.horizontalRowDimensions);
    }
}
    ...
    public transform(
        ...
        for (const row of enabledRows) {
            if (this.grid.horizontalRowDimensions) {
                PivotUtil.flattenGroupsHorizontally(data, row, expansionStates, defaultExpand);
            } else {
        ...
        let finalData = data;
        if (this.grid.horizontalRowDimensions) {
            (this.grid as any)._visibleRowDimensions = PivotUtil.getVisibleRowDimensions(finalData);
        } else {
            (this.grid as any)._visibleRowDimensions = enabledRows;
...
export class IgxPivotGridHorizontalRowCellMerging implements PipeTransform {
    ...
    public transform(
        ...
        const verticalMergeGroups: IPivotGridHorizontalGroup[] = [];
        for (let dimIndex = 0; dimIndex < this.grid.visibleRowDimensions.length; dimIndex++) {
            ...
            for(let i = 1; i < data.length; i++) {
                ...
                if (curRecValue && curRecValue === curGroup.value) {
                    curGroup.rowSpan++;
                    curGroup.records.push(curRec);
                } else {
                    verticalMergeGroups.push(curGroup);
                    curGroup = {
                        colStart: dimIndex + 1,
                        colSpan: 1,
        ...
        // Sort merge groups by rowIndex, so that first are the top group, then on "second" merged row and etc.
        const orderedGroups: IPivotGridHorizontalGroup[][]  = [ ...data.map(_ => []) ];
        for (const group of verticalMergeGroups) {
            // First assign row for each group. Groups are sorted by dimensions by default.
            orderedGroups[group.rowStart - 1].push(group);
        }

        // Merge rows in a single array
        const sortedGroups = orderedGroups.reduce((prev, cur) => prev.concat(...cur), []);
