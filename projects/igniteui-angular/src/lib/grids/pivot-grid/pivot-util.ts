...
import { DimensionValueType, IPivotAggregator, IPivotConfiguration, IPivotDimension, IPivotGridRecord, IPivotKeys, IPivotValue, PivotDimensionType } from './pivot-grid.interface';

export class PivotUtil {

    // go through all children and apply new dimension groups as child
    public static processGroups(recs: IPivotGridRecord[], dimension: IPivotDimension, pivotKeys: IPivotKeys, cloneStrategy: IDataCloneStrategy, horizontalRendering: boolean) {
        for (const rec of recs) {
            // process existing children
            if (rec.children && rec.children.size > 0) {
                // process hierarchy in dept
                rec.children.forEach((values) => {
                    this.processGroups(values, dimension, pivotKeys, cloneStrategy, horizontalRendering);
                });
            }
            if (!horizontalRendering || !rec.children || rec.children.size === 0) {
                ...
                const siblingData = PivotUtil
                    .processHierarchy(hierarchyFields, pivotKeys, horizontalRendering ? rec.level + 1 : 0);
    ...
    public static flattenGroupsHorizontally(data: IPivotGridRecord[],
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const isExpanded = expansionStates.get(expansionRowKey) === undefined ?
                ...
                expansionStates.get(expansionRowKey);
            //const shouldExpand = isExpanded || !dimension.childLevel || !rec.dimensionValues.get(dimension.memberName);
            if (isExpanded && recordsData) {
                if (dimension.childLevel) {
                    this.flattenGroupsHorizontally(recordsData, dimension.childLevel, expansionStates, defaultExpand, dimension, rec);
                } else {
                    // copy parent values and dims in child
                    recordsData.forEach(x => {
                        rec.dimensionValues.forEach((value, key) => {
    ...
    public static getVisibleRowDimensions(data: IPivotGridRecord[]) {
        let visibleRowDims = [];
        data.forEach(rec => {
            if (rec.dimensions.length > visibleRowDims.length) {
                visibleRowDims = rec.dimensions;
            }
        });
        return visibleRowDims;
    }
