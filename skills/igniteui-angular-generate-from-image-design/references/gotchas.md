...
```html
[markerTypes]="<resolvedMarkerTypes>"</resolvedMarkerTypes>
```
...
```html
<igx-category-chart
  [includedProperties]="['fieldOne', 'fieldTwo', 'fieldThree']"
></igx-category-chart>
```
...
[brushes]="'<resolved-series-brush
  >'" [outlines]="'<resolved-series-outline
    >'" [xAxisLabelTextColor]="'<resolved-axis-label-color
      ...
      ></resolved-axis-label-color
    ></resolved-series-outline
  ></resolved-series-brush
>
```
...
```scss
@include theme($palette: $resolved-palette, $schema: <resolved-dark-schema>);
```
