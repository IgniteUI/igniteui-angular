...
xdescribe('IgxGrid - Column Moving', () => {
    ...
    it('Should reorder only movable columns when dropping the ghost image on an interactive area.', fakeAsync(() => {
        ...
        let header = headers[2].nativeElement;
        let columnsList = grid.columnList.toArray();
        ...
        // step 2 - verify columns are reordered when
        // moving a column that is movable
        header = headers[0].nativeElement;
    ...
    it('Should not reorder columns when dropping the ghost image on a non-interactive area.', fakeAsync(() => {
        ...
        let header = headers[1].nativeElement;
        let columnsList = grid.columnList.toArray();
        ...
        fixture.componentInstance.enableRowSelection = true;
        fixture.detectChanges();

        // step 2 - start moving a column, release the drag ghost over
        // the row selector cell and verify columns are not reordered
        header = headers[0].nativeElement;
        UIInteractions.simulatePointerEvent('pointerdown', header, 210, 65);
        UIInteractions.simulatePointerEvent('pointermove', header, 204, 71);
        tick(20);
        UIInteractions.simulatePointerEvent('pointermove', header, 100, 75);
        UIInteractions.simulatePointerEvent('pointerup', header, 100, 75);
        fixture.detectChanges();
    ...
    it('Should not reorder columns on hitting ESCAPE key.', fakeAsync(() => {
        ...
        // step 2 - hit ESCAPE over the cells area and verify column moving is canceled
        ...
        // step 4 - hit ESCAPE over the headers area and verify column moving is canceled
        document.dispatchEvent(new KeyboardEvent('keydown', { 'key': 'Escape' }));
        document.dispatchEvent(new KeyboardEvent('keyup', { 'key': 'Escape' }));
        fixture.detectChanges();
    ...
    it('Should close filter dialog, if opened, when column moving starts.', fakeAsync(() => {
        ...
        expect(dialog.nativeElement.classList.contains('igx-toggle')).toBeFalsy();
    ...
    it('Should be able to scroll/reorder columns that are out of view.', fakeAsync(() => {
        ...
        expect(columnsList[8].field).toEqual('Phone');
        expect(columnsList[9].field).toEqual('ID');
        expect(columnsList[10].field).toEqual('Fax');

        // step 3 - start moving a column and verify columns are scrolled into view,
        // when holding the drag ghost over the left edge of the grid
        header = fixture.debugElement.queryAll(By.css(COLUMN_HEADER_CLASS))[5].nativeElement;
        ...
        expect(columnsList[9].field).toEqual('Phone');
        expect(columnsList[10].field).toEqual('Fax');
    }));
    ...
    it('Should be able to scroll/reorder columns that are out of view - with pinned columns.', fakeAsync(() => {
        ...
        UIInteractions.simulatePointerEvent('pointermove', header, 156, 56);
        tick(20);
        UIInteractions.simulatePointerEvent('pointermove', header, 490, 30);
        tick(4500);
        fixture.detectChanges();
        ...
        expect(columnsList[0].field).toEqual('ID');
        expect(columnsList[8].field).toEqual('Phone');
        expect(columnsList[9].field).toEqual('CompanyName');
        expect(columnsList[10].field).toEqual('Fax');

        // step 3 - start moving a column and verify columns are scrolled into view,
        // when holding the drag ghost before pinned area edge
        header = fixture.debugElement.queryAll(By.css(COLUMN_HEADER_CLASS))[6].nativeElement;
        ...
        expect(columnsList[2].field).toEqual('Fax');
        expect(columnsList[9].field).toEqual('Phone');
        expect(columnsList[10].field).toEqual('CompanyName');
    }));
    ...
    it('Should preserve cell selection after columns are reordered - horizontal scrolling.', fakeAsync(() => {
        ...
        expect(grid.getCellByColumn(0, 'ID')).toBeTruthy();
        expect(grid.getCellByColumn(0, 'CompanyName')).toBeFalsy();


        // step 3 - reorder that column among columns that are currently out of view
        headers = fixture.debugElement.queryAll(By.css(COLUMN_HEADER_CLASS));
        header = headers[4].nativeElement;

        UIInteractions.simulatePointerEvent('pointerdown', header, 250, 50);
        UIInteractions.simulatePointerEvent('pointermove', header, 256, 56);
        tick(20);
        UIInteractions.simulatePointerEvent('pointermove', header, 10, 30);
        tick(4500);
        fixture.detectChanges();
        ...
        // step 4 - verify selection is preserved
        expect(grid.rowList.toArray()[0].cells.toArray()[0].selected).toBeFalsy();
        expect(grid.rowList.toArray()[0].cells.toArray()[1].selected).toBeFalsy();
    ...
    it('Should preserve cell selection after columns are reordered - vertical scrolling.', fakeAsync(() => {
        ...
        const header = headers[5].nativeElement;
        UIInteractions.simulatePointerEvent('pointerdown', header, 350, 50);
        UIInteractions.simulatePointerEvent('pointermove', header, 356, 56);
        tick(20);
        UIInteractions.simulatePointerEvent('pointermove', header, 10, 30);
        ...
        expect(grid.columnList.toArray()[0].cells[4].selected).toBeFalsy();

        grid.verticalScrollContainer.getVerticalScroll().scrollTop = 1200;
        grid.verticalScrollContainer.getVerticalScroll().dispatchEvent(new Event('scroll'));
        fixture.detectChanges();
        expect(grid.columnList.toArray()[0].cells[4].selected).toBeTruthy();
    }));
    ...
    it('Should affect all pages when columns are reordered and paging is enabled.', fakeAsync(() => {
        ...
        // step 4 - navigate to page 1 and verify correct column order
        grid.paginate(0);
        fixture.detectChanges();
    ...
    xit('Pinning - should be able to reorder pinned columns among themselves.', fakeAsync(() => {
        ...
        tick(100);
        fixture.detectChanges();
        UIInteractions.simulatePointerEvent('pointermove', header, 230, 31);
        UIInteractions.simulatePointerEvent('pointerup', header, 230, 31);
        tick(100);
        fixture.detectChanges();
    ...
    it('Pinning - should unpin a pinned column when drag/drop it among unpinned columns.', fakeAsync(() => {
        ...
        // step 4 - drag/drop a pinned column among unpinned columns
        header = fixture.debugElement.queryAll(By.css(COLUMN_HEADER_CLASS))[1].nativeElement;
        UIInteractions.simulatePointerEvent('pointerdown', header, 160, 25);
        UIInteractions.simulatePointerEvent('pointermove', header, 160, 31);
        tick(20);
        UIInteractions.simulatePointerEvent('pointermove', header, 430, 31);
        UIInteractions.simulatePointerEvent('pointerup', header, 430, 31);
        fixture.detectChanges();

        // step 5 - verify column is unpinned at the correct place
        expect(grid.pinnedColumns[0].field).toEqual('ContactTitle');
