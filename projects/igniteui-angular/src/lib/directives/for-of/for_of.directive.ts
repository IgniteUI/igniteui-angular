...
export class IgxGridForOfDirective<T, U extends T[] = T[]> extends IgxForOfDirective<T, U> implements OnInit, OnChanges, DoCheck {
    ...
    constructor() {
        ...
        const _template = inject<TemplateRef<NgForOfContext<T>>>(TemplateRef);
        const _differs = inject(IterableDiffers);
        const cdr = inject(ChangeDetectorRef);
        const _zone = inject(NgZone);
        const _platformUtil = inject(PlatformUtil);
        const _document = inject(DOCUMENT);
        const syncScrollService = inject(IgxForOfScrollSyncService);
