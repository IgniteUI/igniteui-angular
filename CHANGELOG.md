...
- `IgxAvatar`
    - **Breaking Change** The `roundShape` property has been deprecated and will be removed in a future version. Users can control the shape of the avatar by the newly added `shape` attribute that can be either `square`, `rounded` or `circle`. The default shape of the avatar is `square`. 
- `IgxOverlayService`
