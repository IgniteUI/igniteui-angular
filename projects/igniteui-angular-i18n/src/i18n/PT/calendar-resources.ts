import { IGX_PREFIX, prefixResource } from 'igniteui-i18n-core';
import { CalendarResourceStringsPT as ACalendarResourceStrings } from 'igniteui-i18n-resources';

/**
 * Portuguese resource strings for IgxCalendar
 * 
 * Note:  This is an old implementation that uses outdated API for resource management.
 * We recommend using the new API and resources from 'igniteui-i18n-resources'.
 */
export const CalendarResourceStringsPT = prefixResource(IGX_PREFIX, ACalendarResourceStrings);
