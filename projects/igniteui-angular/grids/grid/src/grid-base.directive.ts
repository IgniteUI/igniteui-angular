...
let _gridBaseSheet: CSSStyleSheet | null = null;
...
export abstract class IgxGridBaseDirective implements GridType,
    ...
    */
    ...
    /** @hidden @internal */
    private _registerBaseStyles() {
        if (!this.platform.isBrowser) return;

        const adopted = this.document.adoptedStyleSheets;

        if (_gridBaseSheet === null) {
            _gridBaseSheet = new CSSStyleSheet();
            _gridBaseSheet.replaceSync(GRID_BASE_CSS);
        }

        if (!adopted.includes(_gridBaseSheet)) {
            this.document.adoptedStyleSheets = [...adopted, _gridBaseSheet];
        }
    }
